/**
 * Dimension D E2E harness — comms fire-and-forget (remind_ms=0) mechanism
 * layer: the capability task change notifications are built on.
 *
 * Verifies against a REAL NATS server (no LLM, no tmux spawn):
 *   D-1  fire-and-forget send (remind_ms=0): message delivered (comms_history
...
async function main(): Promise<void> {
  ...
  // ━━ D-1: fire-and-forget send (remind_ms=0) — delivered, NOT tracked ━━━━━━
  // A task change notification is just a plain comms_send with remind_ms=0 —
  ...
  check("D-1a", !!syncedMsgId, `fire-and-forget send delivered (msg=${syncedMsgId})`);
  check("D-1b", !messaging.listPendingReplies().some((p) => p.msg_id === syncedMsgId),
    "remind_ms=0 → NOT in the pending table (auto-exit guard unaffected)");
  check("D-1c", !messaging.listEndedReplies().some((e) => e.msg_id === syncedMsgId),
    "and not in the ended table either — no outbox pollution");
  ...
  check("D-1d", !!historyRec, `comms_history outbound record persisted (${!!historyRec})`);
  if (historyRec) {
    const msg = (historyRec as any).message ?? "";
    check("D-1e", String(msg).includes("item-d1") && String(msg).includes("task_read"),
      "notification body carries the task id + task_read pointer");
  }

  // ━━ D-2: control — plain tracked send IS parked ━━━━━━━━━━━━━━━━━━━━━━━━━━━
  const tracked = await messaging.send(harnessId, "d-recipient", "normal tracked send");
  check("D-2a", messaging.listPendingReplies().some((p) => p.msg_id === tracked.msg_id),
    "remind_ms omitted → parked in the pending table (tracked behavior unchanged)");
  messaging.dismissReply(tracked.msg_id);
