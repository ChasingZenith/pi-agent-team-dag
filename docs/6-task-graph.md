...
- **版本语义**：`version` = 内容修订号——`title / description / deps / subgraph_deps / kind` 任一变化（即每次成功的 `task_commit`）+1；**生命周期事件（状态迁移/派发/开工）与完成报告不 bump**，只追加 history 摘要并更新 `updated_at/by`。快照只对内容版本写。
