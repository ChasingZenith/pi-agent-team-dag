...
- **变更通知**：comms 的无提醒单向消息（`comms_send(..., remind_s=0)`），公告体指向 `task_read`。

---

## 4. 组件装配与协作

- **spawn 统一装配**：spawn 出的 agent 由 launch script 统一加载 comms + task-graph + task-comms-ops + role-context + auto-exit（docs/3 §7.1），自动注册 comms、自带角色模板与任务图工具；交互式启动的 agent 加载组合扩展目录即可——`extensions/agent-lifecycle`、`extensions/teammate-provider` 等入口的 `package.json` `pi.extensions` 清单声明其低层依赖并按顺序自动加载，不需要手写依赖清单（docs/2 §5）
