...
description: Clarifies a vague request against the project's reality, iterating with the user until the requirement is concrete, records it as a module task in the task graph, dispatches it to a Coordinator via the TP, and exits. Does not decompose or execute.
